...
2. Use correct context name:
   ```bash
   ./cluster-bootstrap-cli/cluster-bootstrap-cli bootstrap dev --context docker-desktop
   ```
...
1. Create the secrets file first:
   ```bash
   ./cluster-bootstrap-cli/cluster-bootstrap-cli init --provider sops
   ```
2. Or explicitly provide the file:
   ```bash
   ./cluster-bootstrap-cli/cluster-bootstrap-cli bootstrap dev --secrets-file ./custom-secrets.enc.yaml
   ```
...
**Command:**
```bash
./cluster-bootstrap-cli/cluster-bootstrap-cli -v bootstrap dev
```
...
```bash
./cluster-bootstrap-cli/cluster-bootstrap-cli bootstrap dev --app-path k8s/apps
```
...
1. Reproduce with verbose output: `./cluster-bootstrap-cli/cluster-bootstrap-cli -v bootstrap dev`
2. Collect dry-run output: `./cluster-bootstrap-cli/cluster-bootstrap-cli bootstrap dev --dry-run --dry-run-output /tmp/debug.json`
