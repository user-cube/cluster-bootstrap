...
func CheckKubectlClusterAccess() error {
	...
	cmd := exec.Command(path, "cluster-info") //nolint:gosec // path is validated by exec.LookPath
...
func CheckHelm() error {
	...
	cmd := exec.Command(path, "version", "--short") //nolint:gosec // path is validated by exec.LookPath
	err = cmd.Run()
	if err != nil {
...
func CheckSOPS(encryptionBackend string) error {
	...
	cmd := exec.Command(path, "--version") //nolint:gosec // path is validated by exec.LookPath
	err = cmd.Run()
	if err != nil {
