...
func EnsureGitCryptAttributes(outputDir string) error {
	...
	data, err := os.ReadFile(path) //nolint:gosec // safe: user-provided gitcrypt key file from flag
	...
	f, err := os.OpenFile(path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600) //nolint:gosec // safe: user-provided gitcrypt key file from flag
	if err != nil {
